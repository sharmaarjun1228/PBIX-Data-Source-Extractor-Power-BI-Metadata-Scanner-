...
SQL_DATABASES_PATTERN = re.compile(
    r'Sql\.Databases\(\s*"([^"]+)"',
    re.IGNORECASE
)

NAV_DB_PATTERN = re.compile(
    r'\{\s*\[\s*Name\s*=\s*"([^"]+)"\s*\]\s*\}\[Data\]',
    re.IGNORECASE
)

SCHEMA_ITEM_PATTERN = re.compile(
    r'\{\s*\[\s*Schema\s*=\s*"([^"]+)"\s*,\s*Item\s*=\s*"([^"]+)"\s*\]\s*\}\[Data\]',
    re.IGNORECASE
)
...
def extract_objects_from_sql(sql: str):
    ...
    for pattern in (SQL_FROM_PATTERN, SQL_JOIN_PATTERN, SQL_EXEC_PATTERN):
        for match in pattern.findall(sql):
            objs.add(match.strip())

    # fallback for stored procedure without EXEC
    if not objs:
        words = re.findall(r'[A-Za-z0-9_\[\]\.]+', sql)
        for w in words:
            if "usp" in w.lower():
                objs.add(w)
...
def process_pbix_file(pbix_path: str):
    ...
    for _, rec in pq_df.iterrows():
        ...
        # ---- Sql.Database ----
        m = SQL_DATABASE_PATTERN.search(expression)
        if m:
            server, database = m.group(1), m.group(2)

        # ---- Sql.Databases ----
        m2 = SQL_DATABASES_PATTERN.search(expression)
        if m2:
            server = m2.group(1)

            db_match = NAV_DB_PATTERN.search(expression)
            if db_match:
        ...
        # ---- SQL Extraction ----
        native = NATIVE_QUERY_PATTERN.search(expression)
        query = QUERY_OPTION_PATTERN.search(expression)
...
def run_extraction(folder):
    ...
    summary = (
        df.dropna(subset=["object_name"])
        .groupby(["server", "database", "object_name", "object_type"])
        .agg(pbix_files=("pbix_name", lambda x: ", ".join(sorted(set(x)))))
        .reset_index()
    )

    # ---------- Save ----------
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    output = os.path.join(folder, f"pbix_output_{ts}.xlsx")
